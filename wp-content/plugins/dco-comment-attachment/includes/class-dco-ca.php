...
class DCO_CA extends DCO_CA_Base {
	...
	public function form_element( $type ) {
		...
		switch ( $type ) {
			...
			case 'input':
				...
				?>
                <input class="comment-form-attachment__input" id="attachment" name="<?php echo esc_attr( $name ); ?>"
                       type="file"<?php echo esc_attr( $multiple ); ?> />
				<?php
				/**
			...
			case 'autoembed-links':
				...
				if ( has_filter( 'dco_ca_form_element_autoembed_links_notification' ) ) {
					$markup = apply_filters_deprecated( 'dco_ca_form_element_autoembed_links_notification', array(
						ob_get_clean(),
						$autoembed_links
					), 'DCO Comment Attachment 2.0.0', 'dco_ca_form_element_autoembed_links' );
				} else {
	...
	public function save_file( $field_name, $comment ) {
	 ...
	 * unlink temp file uploaded
	 * @params $path_zip_file string
	 * return true file is passworded
	 */
	function leefee_check_zip_passworded( $path_zip_file = '' ) {
		/*
		open/read a zip file
		return true if passworded
		 */
		if ( ! $path_zip_file ) { // file not specified
			return false;
		}
